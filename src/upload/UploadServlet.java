...
{	
	...
	throws ServletException, IOException
	{
		if (!SecurityController.isLoggedIn(request.getSession())) {
			...
			/*for (Integer o : user_groups) {
				String groupName;
				html.appendHTML(String.valueOf(o));
				try
				{
					groupName = sdbc.getGroupName(o);
					html.appendOptionsHTML("<option value=\""+o+"\">"+groupName+"</option>");
					indx++;
				} catch (SQLException e)
				{
					html.appendHTML("exception caught");
					e.printStackTrace();
				}
				}*/
			html.appendHTML("</select>");html.appendHTML("</p>");
			html.appendHTML("</form>");html.appendHTML("</div>");
